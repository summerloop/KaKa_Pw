...
-(void)markDates:(NSArray *)dates {
    ...
    for (int i = 0; i<[dates count]; i++) {
        //改变标记的颜色
        [colors addObject:[UIColor colorWithRed:93/255.0 green:179/255.0 blue:88/255.0 alpha:1.0]];
    }
...
//        NSLog(@"markDates");
        ...
        //标记label 的宽高坐标设定
        CGRect rectangle = CGRectMake(targetX,targetY,kVRGCalendarViewDayWidth-10,5);
      label  = [[UILabel alloc]initWithFrame:rectangle];
        label.backgroundColor = [UIColor colorWithRed:93/255.0 green:179/255.0 blue:88/255.0 alpha:1.0];
        label.tag=num+1;
        num++;
        [self addSubview:label];
//        [labelArr addObject:(NSData *)label];
//        UIColor *color;
//        
//        if (selectedDate && selectedDateBlock==targetBlock) {
//            //            改变标记label的颜色
//            color = [UIColor colorWithRed:93/255.0 green:179/255.0 blue:88/255.0 alpha:1.0];
...
- (void)removesubView
{
    ...
    for (int i=0; i<num; i++)
    {
        UILabel *label1 = (UILabel *)[self viewWithTag:i+1];
...
-(void)reset {
    ...
    [self updateSize];
    [self setNeedsDisplay];
    [delegate calendarView:self switchedToYear:[currentMonth year] Month:[currentMonth month] targetHeight:self.calendarHeight animated:NO];
}
...
-(void)showNextMonth {
    ...
    if ([delegate respondsToSelector:@selector(calendarView:switchedToYear:Month:targetHeight:animated:)]) [delegate calendarView:self switchedToYear:[currentMonth year] Month:[currentMonth month] targetHeight:self.calendarHeight animated:YES];
...
//刷新视图 在drawRect方法后 重绘需要调用  setNeedsDisplay
-(void)updateSize {
...
{       
    ...
    if (CGRectContainsPoint(rectArrowLeft, touchPoint)) {
        ...
        [self reset];
        if ((todayMonth!=currentMonthIndex) && [delegate respondsToSelector:@selector(calendarView:switchedToYear:Month:targetHeight:animated:)]) [delegate calendarView:self switchedToYear:[currentMonth year] Month:[currentMonth month]  targetHeight:self.calendarHeight animated:NO];
    }
}
// 开始描绘
#pragma mark - Drawing
- (void)drawRect:(CGRect)rect
{
    ...
    NSMutableArray *weekdays = [[NSMutableArray alloc] initWithArray:[NSArray arrayWithObjects:@"周一",@"周二",@"周三",@"周四",@"周五",@"周六",@"周日", nil]];
...
//        选中日期
        if (selectedDate && i==selectedDateBlock) {
            ...
            CGContextAddRect(context, rectangleGrid);
//            选中日期格的背景颜色
//            [UIColor colorWithRed:93/255.0 green:179/255.0 blue:88/255.0 alpha:1.0]
...
//       选中日期的文本颜色
      ...
      if (todayBlock == selectedDateBlock &&selectedDate && i==selectedDateBlock)
        {
        
            CGRect rectangleGrid = CGRectMake(targetX,targetY,kVRGCalendarViewDayWidth+2,kVRGCalendarViewDayHeight+2);
            CGContextAddRect(context, rectangleGrid);
            //            今天的日期的背景颜色
            CGContextSetFillColorWithColor(context, [UIColor colorWithRed:217/255.0 green:80/255.0 blue:78/255.0 alpha:1.0].CGColor);
            CGContextFillPath(context);
            //            今天日期的文本颜色
            CGContextSetFillColorWithColor(context,
                                           [UIColor whiteColor].CGColor);
...
//        NSDictionary *dictionary = [NSDictionary dictionaryWithObjectsAndKeys:@"font":@"", nil]
